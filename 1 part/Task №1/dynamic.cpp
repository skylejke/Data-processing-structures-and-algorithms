...
void count(int arr[], int size, int newelement) { // вставка нового элемента в массив перед элементом, у которого четное количество цифр 
    int d = 0, c;
    for (int i = 0; i < size; i++) {
        if (arr[i] != 0) {
            if (asd(arr[i]) == 0) {
...
int main()
{
    ...
    if (n > 0 && n <= size) {
        ...
        cout << "n должно быть больше нуля и не больше размера массива";
        exit(0);
    }
    ...
    case(2): // Удалить все четные числа массива.
    {
    ...
    case(3): // Найти максимальное число среди элементов массива, расположенных на четных местах.
    {
        maxoneven(arr1, size);
        break;
    }
